...
newfile = '%s/%s_all_data.dat' % (outdir, star)
...
if is_jitter_rv or is_jitter_tr:
    ...
    if is_clustering:
        for o in range(0, n_jrv + 1):
            params_jitter[o] = clustering_fast(dparams_jitter[o], good_index,
                                               nconv)
...
if is_linear_trend != 'f' or is_quadratic_trend != 'f':
    ...
    if is_clustering:
        for o in range(0, 2):
            params_trends[o] = clustering_fast(dparams_trends[o], good_index,
                                               nconv)

#Create the stellar data
mstar = np.random.normal(loc=mstar_mean,
                         scale=mstar_sigma,
                         size=new_nwalkers * nconv)
rstar = np.random.normal(loc=rstar_mean,
                         scale=rstar_sigma,
                         size=new_nwalkers * nconv)
tstar = np.random.normal(loc=tstar_mean,
                         scale=tstar_sigma,
                         size=new_nwalkers * nconv)
...
if is_linear_trend != 'f' or is_quadratic_trend != 'f':
    fit_trends = [
        best_value(params_trends[0], maxloglike, get_value),
        best_value(params_trends[1], maxloglike, get_value)
    ]

#Calculate the final chi2 for each case
log_like_rv, chi2tot_val_rv, dummy = pti.get_loglike(
    mega_time, mega_rv, megax, megay, mega_err, megae, tlab, jrvlab,
    [True, False], flags, t_cad, n_cad, fit_pars, rvs_pars, ldc_pars,
    fit_trends, fit_jrv, fit_jtr)

log_like_tr, dummy, chi2tot_val_tr = pti.get_loglike(
    mega_time, mega_rv, megax, megay, mega_err, megae, tlab, jrvlab,
    [False, True], flags, t_cad, n_cad, fit_pars, rvs_pars, ldc_pars,
    fit_trends, fit_jrv, fit_jtr)
...
if (method == 'mcmc' or method == 'plot'):
    ...
    #Change between b and i
        ...
        w_s_deg, w_s_deg_l, w_s_deg_r = find_vals_perc(w_vec[o] * 180. / np.pi,
                                                       s_factor)
        ...
        trt_vec[o] = np.sqrt((1. + rr_vec[o])**2 -
                             b_vec[o]**2) / (ar_vec[o] * np.sin(i_vec[o]))
        trt_vec[o] = P_vec[o] / np.pi * np.arcsin(
            trt_vec[o]) * ec_factor * 24.0
        tri_vec[o] = np.sqrt((1. - rr_vec[o])**2 -
                             b_vec[o]**2) / (ar_vec[o] * np.sin(i_vec[o]))
        tri_vec[o] = P_vec[o] / np.pi * np.arcsin(
            tri_vec[o]) * ec_factor * 24.0
        ...
        for m in range(0, len(Tpe_vec[o])):
            Tpe_vec[o][m] = pti.find_tp(T0_vec[o][m], e_vec[o][m], w_vec[o][m],
                                        P_vec[o][m])
        ...
        pgra_vec = (P_vec[o] * 24. * 3600.) * (rr_vec[o] /
                                               ar_vec[o])**2 * np.sin(i_vec[o])
        pgra_vec = 2. * np.pi * np.sqrt(1. - e_vec[o]**2) * (
            k_vec[o] * 1.e5) / pgra_vec  #cm/s^2
        ...
        msgra = (k_vec[o] * 1.e5) * np.sqrt(1. - e_vec[o]**2) / np.sin(
            i_vec[o])
        msgra = msgra * (
            (P_vec[o] * 24. * 3600.) / 2. / np.pi / G_cgs)**(1. / 3.)
        ...
        if (fit_tr[o]):
            ...
            if (is_den_a):
                print_values(params[4 + 5], 'rho*^1/3', 'dentrhee' + pl,
                             'g^{1/3}/cm', '${\\rm g^{1/3}\,cm^{-1}}$')
            else:
                print_values(ar_vec[o], 'a/R*', 'ar' + pl, ' ', ' ')
            print_values(rr_vec[o], 'rp/R*', 'rr' + pl, ' ', ' ')
        if (fit_rv[o]):
            print_values(k_vec[o] * 1e3, 'K', 'k' + pl, 'm/s',
                         '${\\rm m\,s^{-1}}$')
        opars.write(
            '-------------------------Derived------------------------------\n')
        if (fit_rv[o]):
            print_values(m_vec[o], 'Mp', 'mp' + pl, 'M_' + unit_mass,
                         '$M_' + usymbol + '$')
        if (fit_tr[o]):
            print_values(r_vec[o], 'Rp', 'rp' + pl, 'R_' + unit_mass,
                         '$R_' + usymbol + '$')
        ...
        if (fit_tr[o]):
            ...
            print_values(Fp, 'Insolation', 'insolation' + pl, 'F_Earth',
                         '${\\rm F_{\\oplus}}$')
            print_values(ds_vec[o], 'rho*', 'denstr' + pl, 'g/cm^3 (transit)',
                         '${\\rm g\,cm^{-3}}$')
            print_values(irho_vec, 'rho*', 'denssp' + pl,
                         'g/cm^3 (stellar paramters)', '${\\rm g\,cm^{-3}}$')
            print_values(pden_vec, 'rho_p', 'denp' + pl, 'g/cm^3',
                         '${\\rm g\,cm^{-3}}$')
            print_values(pgra_vec, 'g_p', 'grap' + pl, 'cm/s^2 (K and Rp/R*)',
                         '${\\rm cm\,s^{-2}}$')
            print_values(pgra_vec2, 'g_p', 'grappars' + pl,
                         'cm/s^2 (planet parameters)', '${\\rm cm\,s^{-2}}$')
            print_values(msgra, 'M_*', 'mspars' + pl,
                         ' solar masses (scaled parameters)', '$M_\odot$')
...
if (total_rv_fit):
    for o in range(0, nt):
        print_values(rv_vec[o], 'Sys. vel. ' + telescopes_labels[o],
                     telescopes_labels[o], 'm/s', '${\\rm m\,s^{-1}}$')
opars.write('--------------------------------------------------------------\n')

if (is_jitter_rv or is_jitter_tr):
    if (total_rv_fit):
        for o in range(0, n_jrv):
            print_values(params_jitter[o] * 1.e3,
                         telescopes_labels[o] + ' jitter',
                         'j' + telescopes_labels[o], 'm/s',
                         '${\\rm m\,s^{-1}}$')
    if (total_tr_fit):
...
if (is_linear_trend != 'f' or is_quadratic_trend != 'f'):
    if (total_rv_fit):
        print_values(params_trends[0] * 1.e3, 'linear trend', 'ltrend',
                     'm/s/days', '${\\rm m\,s^{-1}\,d^{-1}}$')
        print_values(params_trends[1] * 1.e3, 'quadratic trend', 'qtrend',
                     'm/s/days^2', '${\\rm m\,s^{-1}\,d^{-2}}$')
    opars.write(
        '--------------------------------------------------------------\n')
opars.write('\n')
