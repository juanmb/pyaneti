...
if nplanets_rv > 0:
    ...
    time, rv, err, tspe = np.loadtxt(
        'inpy/' + star + '/' + fname_rv[0],
        usecols=(0, 1, 2, 3),
        dtype={
            'names': ('time', 'rv', 'err', 'telescope'),
    ...
    for i in range(nt):
        ...
        for j in range(len(tspe)):
            if tspe[j].decode('utf-8') == telescopes[i]:
                time_dum.append(time[j])
                rv_dum.append(rv[j])
                errs_dum.append(err[j])
    #The *all variables are lists of lists, each list contains
...
if nplanets_tr > 0:
    ...
    for o in range(myn):
        ...
        if my_tr_err == 0:  #The error bars come from the input file
            dummyd, dummyf, dummye = np.loadtxt(
                filename, usecols=columns_tr, comments='#', unpack=True)
        else:  #The error bars are given in the input
            dummyd, dummyf = np.loadtxt(
                filename, usecols=[0, 1], comments='#', unpack=True)
...
if len(span_tr) ==1 and nplanets >1:  #The user did not change this option in
    input_file.py
