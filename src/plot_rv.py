...
def plot_rv_fancy(rv_vector, fname):
    ...
    for j in range(len(tdata)):
        ...
        plt.errorbar(
            ...
            fmt=mark[tel_lab[j]],
            alpha=1.0,
            color=rv_colors[tel_lab[j]],
            markersize=rv_markersize,
            fillstyle=rv_fillstyle)
    for j in range(len(telescopes_labels)):
        plt.errorbar(
            -1,
            0,
            1e-5,
    ...
    for j in range(len(tdata)):
        ...
        plt.errorbar(
            ...
            res[j],
            edata[j],
            label=telescopes_labels[tel_lab[j]],
            fmt=mark[tel_lab[j]],
            alpha=1.0,
            color=rv_colors[tel_lab[j]],
            markersize=rv_markersize,
...
if nplanets > 0:
    ...
    def plot_rv_all_data():
        ...
        #Model curve
        rvy = pti.rv_curve_mp(rvx, 0.0, t0_val, k_val, P_val, e_val, w_val,
                              alpha_val, beta_val)

        rv_dum = [None] * nt
        res_dum_all = [None] * nt
        for j in range(nt):
            res_dum_all[j] = pti.rv_curve_mp(time_all[j], 0.0, t0_val, k_val,
                                             P_val, e_val, w_val, alpha_val,
                                             beta_val)
        ...
        if kernel_rv[0:2] != 'No':
            ...
            evec = mega_err
            m, C = pti.pred_gp(kernel_rv, pk_rv, xvec, yvec, evec, rvx, jrv,
                               jrvlab)
            plt.plot(
                rvx, rvy * cfactor, 'r', alpha=0.7, label='Planetary signal')
            plt.plot(rvx, m * cfactor, 'b', alpha=0.7, label='Gaussian Process')
            plt.plot(
                rvx, (rvy + m) * cfactor, 'k', label='Planetary signal + GP')
        ...
        for j in range(nt):
            ...
            plt.errorbar(
                time_all[j],
                rv_dum[j] * cfactor,
                np.asarray(errs_datas[j]) * cfactor,
                color=rv_colors[j],
                label=telescopes_labels[j],
                fmt=mark[j],
                alpha=1.0,
        ...
        plt.savefig(fname, format='pdf', bbox_inches='tight')
        plt.savefig(
            fname[:-3] + 'png', format='png', bbox_inches='tight', dpi=300)
        plt.close()
    ...
    def plot_rv_mp():
        ...
        for i in range(nplanets):
            ...
            rvx = np.arange(t0_val[i], t0_val[i] + P_val[i] * 0.999,
                            P_val[i] / 4999.)
            rvy = pti.rv_curve_mp(rvx, 0.0, t0_val[i], k_val[i], P_val[i],
            ...
            rv_pi = pti.rv_curve_mp(mega_time, 0.0, t0_val[i], k_val[i],
                                    P_val[i], e_val[i], w_val[i], 0., 0.)
            #This variable has all the planets
            rv_pall = pti.rv_curve_mp(mega_time, 0.0, t0_val, k_val, P_val,
                                      e_val, w_val, 0.0, 0.0)

            #Let us remove all the signals from the data
            res = np.zeros(shape=len(mega_rv))
            for m in range(len(mega_rv)):
                res[m] = mega_rv[m] - v_val[
                    tlab[m]] - rv_pall[m] - alpha_val - beta_val
            ...
            if kernel_rv[0:2] != 'No':
                xvec = mega_time
                yvec = res
                kernel_val, C = pti.pred_gp(kernel_rv, pk_rv, xvec, yvec, evec,
                                            xvec, jrv, jrvlab)
                res = res - kernel_val
            ...
            plot_rv_fancy(
                [p_rv, rvy, p_all, rv_planet_i, evec, ejvec, res, tlab], fname)
