    ...
    for o in range(nplanets):
        if (fit_tr[o]):
            ...
            for m in range(nbands):
                ...
                locale = []
                localt = []
                for n in range(len(megax)):
                    if (trlab[n] == m):
                        localx.append(megax[n])
                        localy.append(megay[n])
                        locale.append(megae[n])
                        localt.append(0)
                tr_vector[m] = plot_parameters_tr(localx, localy, locale,
                                                  localt, pars_tr, rp_val, o, m)
...
def fancy_tr_plot(tr_vector, pnumber):
    fname = '%s/%s%s_tr.pdf' % (outdir, star, plabels[pnumber])
    ...
    for m in range(0, nbands):
        ...
        if (plot_tr_errorbars):
            ...
            plt.plot((xtime[m] - local_T0) * tfc, yflux[m] - deltay,
                     color=tr_colors[m], ms=7, marker=mark_tr[m], alpha=0.8,
                     linewidth=0.)
            plt.plot((xmodel[m] - local_T0) * tfc, fd_ub[m] - deltay, 'k',
                     linewidth=2.0, alpha=1.0)
            plt.errorbar(-x_lim * 0.95, min(yflux[m] - deltay), eflux[m][0],
                         color=tr_colors[m], ms=7, fmt=mark_tr[m], alpha=1.0)
            plt.annotate(bands[m],
                         xy=(-x_lim * 0.85, min(yflux[m] - deltay)),
                         fontsize=fos * 0.7)
            deltay = deltay + dy
            #if ( is_special ):
...
##
    ...
    for m in range(nbands):
        if (plot_tr_errorbars):
            plt.errorbar((xmodel_res[m] - local_T0) * tfc, res_res[m] * 1e6,
                         eflux[m] * 1e6, fmt='.', alpha=0.5)
        else:
    ...
    if (is_plot_std_tr):
        ...
        plt.annotate('$\sigma = $' + trsstr,
                     xy=(x_lim * (0.80), y0 + 1.8 * miy), fontsize=fos * 0.7)
...
def plot_parameters_tr(time, flujo, eflujo, trlab, pars_tr, rp, plabel, bandlab):
    ...
    local_time_d, xtime_d, yflux_d, eflux_d = create_transit_data(
        time, flujo, eflujo, plabel, local_span)
    ...
    fd_ub = pti.flux_tr(xmodel, newtrlab, dparstr,
                        rp[plabel * nbands + bandlab],
                        my_ldc[bandlab * 2:bandlab * 2 + 2], n_cad[bandlab],
                        t_cad[bandlab])
    #Let us create an unbinned model plot
    #fd_ub_unbinned = pti.flux_tr(xmodel,dparstr,my_ldc,1,t_cad)
    fd_ub_unbinned = pti.flux_tr(xmodel, newtrlab, dparstr,
                                 rp[plabel * nbands + bandlab],
                                 my_ldc[bandlab * 2:bandlab * 2 + 2], [1],
                                 t_cad[bandlab])
    ...
    fd_ub_res = pti.flux_tr(xmodel_res, newtrlab, dparstr,
                            rp[plabel * nbands + bandlab],
                            my_ldc[bandlab * 2:bandlab * 2 + 2],
                            n_cad[bandlab], t_cad[bandlab])
    ...
    for p in range(0, nplanets):
        if (p != plabel):
            ...
            fd_ub_total = fd_ub_total + pti.flux_tr(
                local_time, newtrlab, pars_tr[p], rp[p * nbands + bandlab],
                my_ldc[bandlab * 2:bandlab * 2 + 2], n_cad[bandlab],
                t_cad[bandlab])
...
def plot_all_transits():
    ...
    model_flux = pti.flux_tr(megax, [0] * len(megax), pars_tr, rp_val, my_ldc,
                             n_cad, t_cad)
        ...
        if (fit_tr[i]):
            ...
            if (is_plot_all_tr[i]):
                for j in range(0, len(xt)):
                    ...
                    ytm = pti.flux_tr(xtm, trlab, pars_tr, rps, my_ldc, n_cad,
                                      t_cad)

                    fname = '%s/%s%s_transit%d.pdf' % (outdir, star,
                                                       plabels[i], j)
                    n = xt[j][len(xt[j]) - 1] - xt[0][0]
                    n = int(n / P_val[i])
                    #is_err = plot_tr_errorbars
                    #plot_tr_errorbars = True
                    fancy_tr_plot(t0_val[i] + P_val[i] * n, xt[j],
                                  yt[j], et[j], xtm, xt2[j], ytm,
                                  np.array(yt2[j]), ytm, fname)
...
def clean_transits(sigma=10):
    ...
    solution_flux = pti.flux_tr(xvec_model, trlab, pars_tr, rps, my_ldc, n_cad,
                                t_cad)
    ...
    new_model_flux = pti.flux_tr(new_t, trlab, pars_tr, rps, my_ldc, n_cad,
                                 t_cad)
    #New residuals
    ...
    fname = '%s/%s_ligthcurve.pdf' % (outdir, star)
    print('Creating ', fname)
    ...
    out_f = '%s/%s_new_lc.dat' % (outdir, star)
    of = open(out_f, 'w')
