...
def fancy_tr_plot(t0_val,xtime,yflux,errors,xmodel,xmodel_res,flux_model,res_res,\
    ...
    if plot_tr_errorbars:
        plt.errorbar((xtime - local_T0) * tfc, yflux, errors, color=tr_colors,
                     fmt='.', alpha=1.0)
    else:
        plt.plot((xtime - local_T0) * tfc, yflux, 'o', color=tr_colors, ms=7,
                 alpha=0.8)
        y0, yyyy = ax1.get_ylim()
        plt.errorbar(-x_lim * (0.95), y0 + errors[0] * 1.5, errors[0],
                     color=tr_colors, ms=7, fmt='o', alpha=1.0)
        plt.annotate('Error bar', xy=(-x_lim * (0.70), y0 + errors[0] * 1.65),
                     fontsize=fos * 0.7)
    if is_special:
        [
            plt.fill_between((xmodel - local_T0) * tfc, *flux_model[i:i + 2, :],
                             alpha=0.3, facecolor='b') for i in range(1, 6, 2)
        ]
        plt.fill_between((xmodel - local_T0) * tfc, *flux_model[5:7, :],
                         alpha=0.5, facecolor='k')
    if plot_unbinned_model:
        plt.plot((xmodel - local_T0) * tfc, fd_ub_unbinned, 'b', linewidth=2.0,
                 alpha=1.0)
    ...
    step_plot = step_plot + int(
        step_plot % 2)  # now we ensure the result is par
    ...
    plt.tick_params(labelsize=fos, direction='in')
    if plot_tr_errorbars:
        plt.errorbar((xmodel_res - local_T0) * tfc, res_res * 1e6, errors *
                     1e6, color=tr_colors, fmt='.', alpha=1.0)
    else:
        plt.plot((xmodel_res - local_T0) * tfc, res_res * 1e6, 'o',
                 color=tr_colors, ms=7, alpha=0.8)
    ...
    if is_plot_std_tr:
        ...
        plt.annotate('$\sigma = $' + trsstr,
                     xy=(x_lim * (0.80), y0 + 1.8 * miy),
                     fontsize=fos * 0.7)
#  if select_y_tr:
...
def pars_tr_chain(params, nchain):
    ...
    for m in range(0, nplanets):
        ...
        if is_den_a:
            a_val[m] = a_val[0] * (P_val[m] * P_val[m] * 7464960000. * G_cgs /
                                   3.0 / np.pi)**(1. / 3.)
            if m > 0:
...
def plot_transit_nice():
    ...
    for o in range(0, nplanets):
        ...
        if fit_tr[o]:
            ...
            local_time_d, xtime_d, yflux_d, eflux_d = create_transit_data(
                megax, megay, megae, o, local_span)
            ...
            if is_special_plot_tr:
                ...
                for l in range(0, nc):
                    ...
                    #The model has T0 = 0
                    dumtp = pti.find_tp(0.0, lpars_tr[2, o], lpars_tr[3, o],
                                        lpars_tr[1, o])
                    dparstr = np.concatenate([[dumtp], lpars_tr[1:, o]])
                    #This is the flux of the actual planet
                    flux_vector[l] = pti.flux_tr(xmodel, dparstr, my_ldc,
                                                 n_cad, t_cad)
                    flux_vector_res[l] = pti.flux_tr(xmodel_res, dparstr,
                                                     my_ldc, n_cad, t_cad)

                flux_vector = np.array(flux_vector)
                flux_vector_res = np.array(flux_vector_res)
                fd_ub = np.percentile(flux_vector,
                                      [50, 0.15, 99.85, 2.5, 97.5, 16, 84], 0)
                flux_pc_res = np.percentile(
                    flux_vector_res, [50, 0.15, 99.85, 2.5, 97.5, 16, 84], 0)
            ...
            for p in range(0, nplanets):
                if p != o:
                    ...
                    fd_ub_total = fd_ub_total + pti.flux_tr(
                        local_time, pars_tr[:, p], my_ldc, n_cad, t_cad)
...
def plot_all_transits():
    ...
    for i in range(0, nplanets):
        if fit_tr[i]:
            ...
            xt, dt, yt, et = create_transit_data(megax, megay, megae, i,
                                                 span_tr[i])
            xt2, dt2, yt2, et2 = create_transit_data(megax, res_flux, megae, i,
                                                     span_tr[i])
            ...
            if is_plot_all_tr[i]:
                for j in range(0, len(xt)):
                    ...
                    fname = outdir + '/' + star + plabels[
                        i] + '_transit' + str(j) + '.pdf'
                    n = xt[j][len(xt[j]) - 1] - xt[0][0]
                    n = int(n / P_val[i])
                    #is_err = plot_tr_errorbars
                    #plot_tr_errorbars = True
                    fancy_tr_plot(t0_val[i] + P_val[i] * n, xt[j], yt[j],
                                  et[j], xtm, xt2[j], ytm, np.array(yt2[j]),
                                  ytm, fname)
...
def plot_rv_fancy(p_rv, rvy, p_all, rv_dum, errs_all, res, telescopes_labels,
                  fname, is_special=False):
    print('Creating ', fname)
    if not is_special:
        rv_model = rvy
    else:
    ...
    for j in range(0, nt):
        plt.errorbar(p_all[j],rv_dum[j],new_errs_all[j], fmt=mark[j],
                     alpha=1.0, color='#C0C0C0', markersize=rv_markersize,
                     fillstyle=rv_fillstyle)

        plt.errorbar(p_all[j],rv_dum[j],errs_all[j],
                     label=telescopes_labels[j], fmt=mark[j], alpha=1.0,
                     color=rv_colors[j], markersize=rv_markersize,
                     fillstyle=rv_fillstyle)

    plt.plot(p_rv, rv_model, 'k', linewidth=2.0, alpha=0.9, zorder=3)

    if is_rv_legend:
        plt.legend(loc=2, ncol=1, scatterpoints=1, numpoints=1, frameon=True,
                   fontsize=fos * 0.7)
    ...
    plt.tick_params(axis='x', which='minor', direction='in', bottom=True,
...
if nplanets > 0:
    ...
    def plot_rv_all_data():
        ...
        #Model curve
        rvy = pti.rv_curve_mp(rvx, 0.0, t0_val, k_val*cfactor, P_val, e_val,
                              w_val, alpha_val*cfactor, beta_val*cfactor)
        rv_dum = [None] * nt
        res_dum_all = [None] * nt
        for j in range(0, nt):
            res_dum_all[j] = pti.rv_curve_mp(time_all[j], 0.0, t0_val,
                                             k_val*cfactor, P_val, e_val,
                                             w_val, alpha_val*cfactor,
                                             beta_val*cfactor)
        ...
        plt.tick_params(labelsize=fos, direction='in')
        for j in range(0, nt):
            plt.errorbar(time_all[j], rv_dum[j], new_errs_all[j],
                         color='#C0C0C0', fmt=mark[j], alpha=1.0,
                         markersize=rv_markersize)

            plt.errorbar(time_all[j], rv_dum[j], errs_datas[j],
                         color=rv_colors[j], label=telescopes_labels[j],
                         fmt=mark[j], alpha=1.0, markersize=rv_markersize)
        if is_rv_legend:
            plt.legend(loc=2, ncol=1, scatterpoints=1, numpoints=1,
                       frameon=True, fontsize=fos * 0.8)
...
#===========================================================
    ...
    def plot_rv_mp():
        ...
        for i in range(0, nplanets):
            ...
            rvx = np.arange(t0_val[i], t0_val[i] + P_val[i] * 0.999,
                            P_val[i] / 4999.)
            rvy = pti.rv_curve_mp(rvx, 0.0, t0_val[i], k_dum[i], P_val[i],
                                  e_val[i], w_val[i], 0.0 ,0.0)
            ...
            for j in range(0, nt):  #Remove the signal for each telescope
                ...
                if is_linear_trend or is_quadratic_trend:
                    for m in range(0, len(time_all[j])):
                        alpha_time[m] = (time_all[j][m] -
                                         t0_val[0])**1 * alpha_val * cfactor
                        beta_time[m] = (time_all[j][m] -
                                        t0_val[0])**2 * beta_val * cfactor

                #the actual value, minus the systemic velocity, minus the other planets
                for o in range(len(time_all[j])):
                    if nplanets > 1:
                        rv_dum[j][o] = rv_dum[j][o] - v_val[j] - drvy[j][o] - \
                            alpha_time[o] - beta_time[o]
                    else:
                        rv_dum[j][o] = rv_dum[j][o] - v_val[j] - drvy[j] - \
                            alpha_time[o] - beta_time[o]
            ...
            plot_rv_fancy(p_rv, rvy, p_all, rv_dum, errs_all, res,
                          telescopes_labels, fname, is_special_plot_rv)
...
def plot_posterior():
    create_plot_posterior(params[4:], labels, cbars='red', nb=50,
                          num=plot_parameters)


def create_plot_posterior(params, plabs, cbars='red', nb=50, num=[]):
    n = num if len(num) >= 2 else list(range(0, len(params)))
    ...
    gs = gridspec.GridSpec(
        nrows=int((len(n) + n_columns_posterior - 1) / n_columns_posterior),
        ncols=n_columns_posterior)
    ...
    for i in n:
        ...
        if is_plot_prior:
            ...
            for k in range(0, len(locx)):
                if priorf[i] == 'u':
                    lp[k] = pti.uniform_prior(priorl[i * 2], priorl[i * 2 + 1],
                                              locx[k])
                if priorf[i] == 'g':
                    lp[k] = pti.gauss_prior(priorl[i * 2], priorl[i * 2 + 1],
                                            locx[k])
        ...
        #
        if i == n[0]:
            plt.legend(loc=0, ncol=1, scatterpoints=1, numpoints=1,
                       frameon=True, fontsize=fos * 0.5)
        j = j + 1
