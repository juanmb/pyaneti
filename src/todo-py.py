...
def sigma_clip(x, y, z, limit_sigma=5, is_plot=False):
    ...
    while (control):
        ...
        if len(dummy_x) == len(new_x):
            #We did not cut, so the sigma clipping is done
            control = False
        new_y = list(dummy_y)
        new_x = list(dummy_x)
        new_z = list(dummy_z)
...
def print_values(vector, var, vartex, unit, unittex):
  ...
  {' + str(round(medv, nd)) + ' _{ - ' + str(round(minv, nd)) + ' } ^ { + ' +
               str(round(maxv, nd)) + ' }$~#1} \n')
    if is_print_mode:
...
def joint_fit():
    ...
    for o in range(0, nplanets):
        dummy_lims[o*8*2:(o+1)*8*2 ] = [
            min_t0[o], max_t0[o], min_P[o], max_P[o], min_e[o], max_e[o],
            min_w[o], max_w[o], min_i[o], max_i[o], min_a[o], max_a[o],
            min_rp[o], max_rp[o], min_k[o], max_k[o]
        ]
